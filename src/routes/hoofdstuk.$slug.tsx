...
function ChapterPage(){const chapter=Route.useLoaderData(); const index=chapters.findIndex(c=>c.slug===chapter.slug); const chapterClaims=claims.filter(c=>c.chapter===chapter.nr); const sourceIds=[...new Set(chapterClaims.flatMap(c=>c.sources))]; return <SiteShell><main>
  ...
  <nav className="chapter-nav">{index>0?<Link to="/hoofdstuk/$slug" params={{slug:chapters[index-1].slug}}><ArrowLeft/> <span><small>Vorige</small>{chapters[index-1].title}</span></Link>:<Link to="/"><ArrowLeft/><span><small>Terug</small>Voorpagina</span></Link>}{index<chapters.length-1?<Link className="next" to="/hoofdstuk/$slug" params={{slug:chapters[index+1].slug}}><span><small>Volgende</small>{chapters[index+1].title}</span><ArrowRight/></Link>:<Link className="next" to="/bronnen"><span><small>Verder</small>Bronnenregister</span><ArrowRight/></Link>}</nav>
  </main></SiteShell>}